...
export default function ConsultaSGP() {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-slate-50 overflow-hidden relative">
      {/* Header */}
      <div className="p-6 border-b border-slate-200 bg-white z-10 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 font-outfit flex items-center gap-2">
            <Server className="text-blue-600" size={24} />
            Workspace ERP (Integração SGP)
          </h1>
          <p className="text-sm text-slate-600 mt-1">Visão 360 do assinante: Suporte, Financeiro e Vendas num único painel.</p>
        </div>
      </div>

      <div className="flex-1 overflow-y-auto p-6 md:p-8">
        {/* Search Bar */}
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 mb-8 max-w-5xl mx-auto">
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-20 text-blue-600">
            ...
            <p className="font-bold text-slate-600">Sincronizando com SGP...</p>
          </div>
        )}
        ...
        {!loading && resultados && resultados.length > 0 && (
          <div className="max-w-5xl mx-auto animate-in fade-in slide-in-from-bottom-4">
            {resultados.map((res: any) => (
              <div key={res.id} className="bg-white rounded-3xl border border-slate-200 shadow-lg shadow-slate-200/40 overflow-hidden">
                
                {/* Profile Header */}
                <div className="p-8 border-b border-slate-200 bg-gradient-to-br from-slate-900 to-slate-800 text-white relative overflow-hidden">
                  ...
                  <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                    ...
                    <div className="flex gap-4">
                       <div className="bg-white/10 backdrop-blur-md border border-white/10 rounded-xl p-4 min-w-[120px]">
                ...
                <div className="p-8">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                      ...
                      <div className="lg:col-span-1 space-y-6">
                        <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200 shadow-inner">
                          <h3 className="font-bold text-slate-900 font-outfit mb-4 text-sm flex items-center gap-2 border-b border-slate-200 pb-2">
                            <User size={16} className="text-blue-600" /> Dados de Contato
                          </h3>
                          <div className="space-y-4">
                            <div>
                              ...
                              <div className="flex items-center justify-between bg-white border border-slate-200 px-3 py-2 rounded-lg">
                                <span className="font-bold text-slate-900">{res.contato.telefone}</span>
                                <button className="text-emerald-600 hover:text-emerald-700 bg-emerald-50 px-2 py-1 rounded text-xs font-bold flex items-center gap-1 transition-colors">
                            ...
                            <div>
                              ...
                              <p className="font-medium text-slate-900 text-sm bg-white border border-slate-200 px-3 py-2 rounded-lg truncate">{res.contato.email}</p>
                            </div>
                            <div>
                              ...
                              <div className="bg-white border border-slate-200 px-3 py-2.5 rounded-lg space-y-2">
                                <p className="font-medium text-slate-900 text-sm leading-relaxed">{res.endereco}</p>
                                ...
                                <div className="flex items-center gap-2 pt-1 border-t border-slate-100">
                                  <button
                                    type="button"
                                    onClick={() => setMapTargetCliente(res)}
                                    className="px-2.5 py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 rounded-md text-xs font-bold flex items-center gap-1 transition-colors shadow-2xs"
                                  >
                                    <MapPin size={12} />
                      ...
                      <div className="lg:col-span-2">
                         <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                           <h3 className="font-bold text-slate-900 font-outfit mb-6 text-sm flex items-center gap-2 border-b border-slate-100 pb-3">
                             <Activity size={16} className="text-blue-600" /> Histórico de Atendimentos (Últimos 30 dias)
                           </h3>
                               ...
                               <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
                                 <div className="flex justify-between items-start mb-2">
                                   <div className="flex items-center gap-2">
                                     <span className="bg-slate-200 text-slate-700 text-[10px] px-2 py-0.5 rounded font-bold">Ticket #{chamado.id}</span>
                                 ...
                                 <p className="text-sm font-medium text-slate-900">{chamado.assunto}</p>
                               </div>
                             </div>
                           ))}
                         </div>
                        ...
                        <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200 shadow-inner">
                           ...
                           <div className="space-y-3">
                             ...
                             {res.status_cliente === 'bloqueado_parcial' && (
                               <button className="w-full bg-amber-50 hover:bg-amber-100 border border-amber-200 text-amber-700 py-3 rounded-xl font-bold transition-all text-sm flex items-center justify-center gap-2">
                                 <ShieldCheck size={16} /> Desbloqueio em Confiança (48h)
                               </button>
                             )}
                             <button className="w-full bg-white hover:bg-slate-100 border border-slate-200 text-slate-700 py-3 rounded-xl font-bold transition-all text-sm flex items-center justify-center gap-2">
                               <FileText size={16} /> Promessa de Pagamento
                             </button>
                           </div>
                        </div>
                      </div>
                      ...
                      <div className="lg:col-span-2 space-y-4">
                        {res.faturas.map((fatura: any) => (
                          <div key={fatura.id} className={`p-5 rounded-2xl border ${
                            fatura.status === 'atrasado' ? 'bg-red-50/30 border-red-200' : 'bg-white border-slate-200'
                            ...
                            <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-4">
                              <div className="flex items-center gap-4">
                                ...
                                <div>
                                  <div className="flex items-center gap-2 mb-1">
                                    <h4 className="font-bold text-slate-900 text-lg">Fatura #{fatura.id}</h4>
                              ...
                              <div className="text-right">
                                <p className="text-2xl font-bold font-outfit text-slate-900">R$ {fatura.valor.toFixed(2).replace('.', ',')}</p>
                              ...
                              <div className="pt-4 border-t border-slate-200 mt-2">
                                ...
                                <div className="flex gap-2">
                                  <input 
                                    ...
                                    className="flex-1 bg-white border border-slate-200 rounded-lg px-3 py-2 text-xs font-mono text-slate-600 outline-none"
                                  />
                                  <button 
                                    onClick={() => copyToClipboard(fatura.linha_digitavel)}
                                    className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg font-bold text-xs flex items-center gap-2 transition-colors border border-slate-200"
                                  >
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                      <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200 shadow-inner">
                        <div className="flex justify-between items-center border-b border-slate-200 pb-4 mb-4">
                          <h3 className="font-bold text-slate-900 font-outfit text-sm flex items-center gap-2 uppercase tracking-wider">
                            <Router size={16} className="text-blue-600" /> Detalhes da Conexão (Radius)
                          </h3>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                            ...
                            <p className="font-bold text-slate-900 text-sm">{res.conexao.plano}</p>
                          </div>
                          <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                            <p className="text-[10px] uppercase tracking-wider font-bold text-slate-500 mb-1">Uptime</p>
                            <p className="font-bold text-slate-900 text-sm">{res.conexao.uptime}</p>
                          </div>
                          <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                        ...
                        <div className="mt-6 flex gap-3">
                           <button className="flex-1 bg-amber-50 hover:bg-amber-100 border border-amber-200 text-amber-700 py-3 rounded-xl font-bold transition-all text-sm flex items-center justify-center gap-2 shadow-sm">
                             <Zap size={16} /> Kick (Derrubar Conexão)
                           </button>
                           <button className="flex-1 bg-white hover:bg-slate-100 border border-slate-200 text-slate-700 py-3 rounded-xl font-bold transition-all text-sm flex items-center justify-center gap-2 shadow-sm">
                  ...
                  {activeTab === 'vendas' && (
                    <div>
                      <div className="mb-6">
                        <h3 className="font-bold text-slate-900 font-outfit text-lg flex items-center gap-2">
                          <ArrowUpRight className="text-blue-600" /> Oportunidades de Upgrade (Cross-sell)
                        </h3>
                        <p className="text-slate-600 text-sm">Com base no consumo de {res.metricas.consumo_mes_gb}GB/mês, estes planos são recomendados.</p>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl">
                        {res.planos_disponiveis.map((plano: any) => (
                          <div key={plano.id} className={`p-6 rounded-2xl border-2 transition-all cursor-pointer hover:shadow-lg ${
                            plano.destaque ? 'border-blue-600 bg-blue-50/30' : 'border-slate-200 bg-white hover:border-blue-300'
                          }`}>
                            {plano.destaque && (
                              <span className="inline-block bg-blue-600 text-white text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-md mb-4 shadow-sm">
                                Recomendado pela IA
                              </span>
                            )}
                            <div className="flex justify-between items-start mb-2">
                              <h4 className="text-2xl font-bold text-slate-900 font-outfit">{plano.nome}</h4>
                            </div>
                            <p className="text-3xl font-bold font-outfit text-slate-900 mb-6">
                            ...
                            <button className={`w-full py-3 rounded-xl font-bold transition-all shadow-sm ${
                              plano.destaque ? 'bg-blue-700 hover:bg-blue-800 text-white shadow-blue-700/20' : 'bg-slate-100 hover:bg-slate-200 text-slate-800 border border-slate-200'
                            }`}>
                              Ofertar Upgrade
                            </button>
                          </div>

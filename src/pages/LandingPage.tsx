...
export default function LandingPage() {
  ...
  const [activeTemplate, setActiveTemplate] = useState<1 | 2 | 3>(
    (config.landingPage?.templatePadrao as 1 | 2 | 3) || 1
  );
  const [isConfigOpen, setIsConfigOpen] = useState(false);

  // Sincroniza com a preferência gravada no admin se houver alteração
  useEffect(() => {
    if (config.landingPage?.templatePadrao) {
      setActiveTemplate(config.landingPage.templatePadrao as 1 | 2 | 3);
    }
  ...
  return (
    <>
      {activeTemplate === 1 && <Template1 config={config} />}
      {activeTemplate === 2 && <Template2 config={config} />}
      {activeTemplate === 3 && <Template3 config={config} />}

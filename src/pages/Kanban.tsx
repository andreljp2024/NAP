...
export default function Kanban({ type }: { type: "Suporte" | "Vendas" }) {
  ...
  useEffect(() => {
    // In a real app, this calls our Express server which queries PostgreSQL
    fetch('/api/deals')
      .then(res => res.json())
      .then((data: Deal[]) => {
            ...
            return (
              <div key={stage} className="w-80 flex flex-col max-h-full">
                ...
                <div className="flex-1 overflow-y-auto space-y-3 pb-4">
                  {stageDeals.map(deal => (
                    <div key={deal.id} className="bg-white p-4 rounded-lg border border-slate-200 shadow-sm hover:shadow-md transition-shadow cursor-grab">

...
import { FileText, CheckCircle2, Clock, Download, QrCode, Loader2, Copy, Zap, ShieldCheck, AlertCircle, Sparkles } from 'lucide-react';
  ...
  const handleSolicitarDesbloqueio = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        localStorage.setItem('nap_desbloqueio_48h', dataExpiracao.toISOString());
        setDesbloqueioAtivo(true);
        setDesbloqueioMsg("Desbloqueio em confiança realizado! Sua conexão foi reativada no concentrador por 48 horas.");
      }
    } catch {
  ...
  return (
    <div className="p-4 md:p-8 max-w-4xl mx-auto w-full">
      <div className="mb-6 md:mb-8">
        <h1 className="text-2xl md:text-3xl font-bold text-slate-900 font-outfit mb-2">Faturas</h1>
      ...
      {desbloqueioAtivo ? (
        <div className="mb-6 p-4 rounded-2xl bg-emerald-50 border border-emerald-200 flex items-center justify-between gap-3 animate-in fade-in">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-bold text-emerald-950">Desbloqueio em Confiança Ativo (48 Horas)</p>
        ...
        <div className="mb-6 p-4 rounded-2xl bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3 animate-in fade-in">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-amber-800">Você tem direito a 1 liberação temporária de 48 horas enquanto processa o pagamento da sua fatura.</p>
            </div>
          </div>
          <button
              ...
              <div key={fatura.id} className="p-5 md:p-6 hover:bg-slate-100/40 transition-colors group">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-3">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <h3 className="font-bold text-slate-900 font-outfit">Mensalidade - Fibra 500MB</h3>
                  ...
                  <div className="flex flex-col items-end gap-4 w-full md:w-auto mt-2 md:mt-0">
                    ...
                    {fatura.status === 'pago' && (
                      <button 
                        onClick={() => handleGenerateBoleto(fatura.id)}
                        className="w-full md:w-auto bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-600 px-5 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2"
                      >

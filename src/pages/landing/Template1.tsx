...
export default function Template1({ config: propsConfig }: TemplateProps) {
  ...
  const nomeProvedor = config?.provedor?.nomeFantasia || "NAP Fibra";
        ...
        <header className="fixed top-0 w-full z-50 bg-[#060b14]/80 backdrop-blur-md border-b border-white/5">
          <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
              ...
              {logoUrl ? (
                ...
                <div className="flex items-center gap-2">
                  <div className="w-10 h-10 rounded-xl bg-blue-600 flex items-center justify-center shadow-lg shadow-blue-600/30">
                    <span className="font-bold text-xl text-white">{(nomeProvedor || "N")[0]}</span>
                  </div>
            ...
            <nav className="hidden md:flex gap-8 text-sm font-medium text-slate-300">
              ...
              <a href={`https://wa.me/55${zapClean}`} target="_blank" rel="noreferrer" className="hover:text-white transition-colors">WhatsApp Vendas</a>
            </nav>
        ...
        <section className="relative pt-32 pb-20 lg:pt-48 lg:pb-32 px-6 overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto relative z-10 text-center">
            ...
            >
              ...
              <Link to="/portal" className="w-full sm:w-auto bg-white/10 hover:bg-white/15 text-white px-8 py-4 rounded-full font-bold text-base sm:text-lg transition-all border border-white/20 hover:border-white/30 flex items-center justify-center gap-2">
        ...
        <section className="py-24 px-6" id="planos">
          <div className="max-w-7xl mx-auto">
            <div className="text-center mb-16">
              ...
              <p className="text-slate-400">Escolha a velocidade ideal para a sua residência ou empresa.</p>
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
              {planos.map((plan, i) => (
                ...
                >
                  {plan.popular && (
                    <div className="absolute top-6 right-0 bg-white text-blue-600 text-xs font-bold px-3 py-1 rounded-l-full uppercase tracking-wider shadow">
                      {plan.tag}
                    </div>
                  )}
                  <div className="bg-[#0b1120] rounded-[22px] p-8 h-full flex flex-col">
                    ...
                    <a 
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className={`w-full py-4 rounded-xl font-bold transition-all text-center flex items-center justify-center gap-2 ${plan.popular ? 'bg-blue-600 hover:bg-blue-500 text-white shadow-lg shadow-blue-600/30' : 'bg-white/5 hover:bg-white/10 text-white'}`}
                    >
                      Assinar Plano
                    </a>
                  </div>

...
export default function PortalSpeedtestModal({
  ...
  const [serverInfo, setServerInfo] = useState({
    nome: 'NAP Telecom • PTT São Paulo (IX.br)',
    ip: '177.85.112.44',
    distancia: '4.2 km'
  });
      ...
      <div className="bg-[#101726] border border-white/10 rounded-3xl w-full max-w-lg p-6 shadow-2xl relative overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between pb-4 border-b border-white/5 relative z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-white text-base font-outfit">Speedtest do Provedor</h3>
              <p className="text-xs text-slate-400">Medição direta no Servidor de Borda / IX.br</p>
            </div>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="mt-4 p-3 bg-white/[0.02] border border-white/5 rounded-2xl flex items-center justify-between text-xs relative z-10">
          <div className="flex items-center gap-2">
            <Server size={14} className="text-blue-400" />
            <div>
              <span className="text-white font-medium block">{serverInfo.nome}</span>
              <span className="text-[10px] text-slate-400 font-mono">IP: {serverInfo.ip} • Distância: {serverInfo.distancia}</span>
            </div>
          </div>
        ...
        <div className="my-6 flex flex-col items-center justify-center relative">
          <div className="relative w-64 h-36 flex items-end justify-center overflow-hidden">
            ...
            <div className="flex flex-col items-center justify-center pb-2 relative z-10">
              ...
              <span className="text-xs font-mono font-bold uppercase tracking-wider text-slate-400 mt-0.5">
                Mbps
              </span>
            </div>
          </div>
        ...
        <div className="grid grid-cols-4 gap-2 mb-4 relative z-10">
          ...
          <div className="bg-[#0b0f19] border border-white/5 rounded-2xl p-3 text-center">
            ...
            <span className="text-[9px] text-slate-500 font-mono">Mbps</span>
          </div>
          ...
          <div className="bg-[#0b0f19] border border-white/5 rounded-2xl p-3 text-center">
            ...
            <span className="text-[9px] text-slate-500 font-mono">ms</span>
          </div>
        ...
        {/* Botões de Ação */}
        <div className="pt-2 flex items-center justify-between gap-3 border-t border-white/5 relative z-10">
          <button
            onClick={startSpeedtest}
            disabled={phase !== 'completed' && phase !== 'idle'}
            className="px-4 py-2.5 bg-white/5 hover:bg-white/10 text-white rounded-xl text-xs font-bold transition-all border border-white/10 flex items-center gap-2 disabled:opacity-50"
          >
          ...
          <div className="flex items-center gap-2">
            {phase === 'completed' && (
              <button
                ...
                className="px-3.5 py-2.5 bg-white/5 hover:bg-white/10 text-slate-300 hover:text-white rounded-xl text-xs font-bold transition-all border border-white/10 flex items-center gap-1.5"
              >

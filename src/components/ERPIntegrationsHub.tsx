          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                O NAP conecta de forma nativa ao ERP do seu provedor para sincronizar assinantes, gerar PIX, consultar radius e liberar conexões em confiança.
              </p>
            </div>
          </div>
        </div>
                ...
                >
                  <div>
                    ...
                    <div className="flex items-start justify-between gap-3 mb-3">
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-xl bg-gradient-to-tr ${erp.corBadge} flex items-center justify-center text-white font-black text-xs shadow`}>
                          {erp.sigla}
                        </div>
                        <div>
                    ...
                    <div className="mb-4">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {erp.recursos.slice(0, 3).map((rec, idx) => (
                          <span key={idx} className="text-[10px] bg-white/5 text-slate-300 px-2 py-0.5 rounded-md border border-white/5">
                            {rec}
                          </span>
                        ))}
                  ...
                  <div className="pt-3 border-t border-white/5 flex items-center gap-2">
                    <button
                      ...
                      className="flex-1 py-2 px-3 bg-white/5 hover:bg-white/10 text-slate-200 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5"
                    >
                      <Sliders size={13} />
                    ...
                    {!isAtivo ? (
                      <button
                        ...
                        title="Tornar este o ERP principal do provedor no NAP"
                      >
                        <Check size={14} />
                        <span>Ativar</span>
                      </button>
              ...
              <div className="p-6 rounded-2xl bg-slate-900 border border-white/5 space-y-5">
                <div className="flex items-center justify-between pb-4 border-b border-white/5">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400">{erpSelecionadoObj.categoria} • Versão {erpSelecionadoObj.versaoApiHomologada}</p>
                    </div>
                  </div>
                    ...
                    return (
                      <div key={campo.key}>
                        <div className="flex items-center justify-between mb-1.5">
                          ...
                          <span className="text-[11px] text-slate-500">{campo.ajuda}</span>
                        </div>
                ...
                <div className="p-4 rounded-xl bg-slate-950 border border-white/5 space-y-3">
                  ...
                  <label className="flex items-start gap-3 cursor-pointer">
                    ...
                    />
                    <div>
                      <span className="text-xs font-bold text-white block">Auto-Desbloqueio 48h em Confiança</span>
                      <span className="text-[11px] text-slate-400">Permite que assinantes com bloqueio financeiro reativem o sinal via Portal ou WhatsApp sem intervenção humana.</span>
                    </div>
                  </label>
                  ...
                  <label className="flex items-start gap-3 cursor-pointer">
                    ...
                    />
                    <div>
                      <span className="text-xs font-bold text-white block">Alerta de Inadimplência na Tela do Operador</span>
                      <span className="text-[11px] text-slate-400">Exibe indicador visual em vermelho no Inbox e no CRM quando o cliente que chama possui fatura em atraso.</span>
                    </div>
                  </label>
                  ...
                  <label className="flex items-start gap-3 cursor-pointer">
                    ...
                    />
                    <div>
                      <span className="text-xs font-bold text-white block">Consulta de Sessão RADIUS (PPPoE / IP / MAC)</span>
                      <span className="text-[11px] text-slate-400">Lê o IP atribuído, MAC Address da ONU e tempo de conexão diretamente do servidor de acesso do provedor.</span>
                    </div>
                  </label>
                </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      disabled={saving}
                      className="px-4 py-2.5 bg-white/5 hover:bg-white/10 text-white rounded-xl text-xs font-bold transition-all flex items-center gap-1.5"
                    >
            ...
            <div className="space-y-5">
              <div className="p-5 rounded-2xl bg-slate-900 border border-white/5 space-y-4">
                ...
                {testResult ? (
                      ...
                      <div className="p-3 rounded-xl bg-slate-950 border border-white/5 space-y-2">
                        ...
                        <div className="text-xs space-y-1 font-mono">
                          <div className="flex justify-between text-slate-400">
                            <span>Cliente:</span>
                            <span className="text-white font-bold">{testResult.exemploSincronizado.cliente_exemplo}</span>
                          </div>
                          <div className="flex justify-between text-slate-400">
                            <span>Contrato:</span>
                            <span className="text-white">{testResult.exemploSincronizado.contrato_codigo}</span>
                          </div>
                          <div className="flex justify-between text-slate-400">
                            <span>Plano:</span>
                            <span className="text-white">{testResult.exemploSincronizado.plano}</span>
                          </div>
                          <div className="flex justify-between text-slate-400">
                  ...
                  <div className="py-10 text-center text-slate-500 space-y-2">
                    <Cpu size={28} className="mx-auto text-slate-600" />
                    <p className="text-xs">
                      Clique em <strong className="text-slate-400">"Validar Pré-Configuração"</strong> para checar em tempo real o handshake SSL, autenticação do token, módulo financeiro e desbloqueio.
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="p-4 rounded-2xl bg-blue-950/20 border border-blue-500/20 space-y-2">
                ...
                <p className="text-[11px] text-slate-400 leading-relaxed">
                  Para permitir que o NAP acesse o webservice do seu ERP sem bloqueios, adicione o IP da sua VM NAP na whitelist de acessos remotos (portas 443 HTTPS / 80 HTTP).
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div className="space-y-1.5">
              ...
              {erpsList.map((e) => (
                ...
                >
                  <div className="flex items-center gap-2.5">
                    <div className={`w-5 h-5 rounded bg-gradient-to-tr ${e.corBadge} flex items-center justify-center text-[9px] text-white font-bold`}>
                      {e.sigla.slice(0, 2)}
                    </div>
                    <span>{e.nome}</span>
                  </div>

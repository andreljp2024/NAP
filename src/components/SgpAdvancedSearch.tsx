...
export default function SgpAdvancedSearch({
  ...
  // Tab interna: 'financeiro' | 'planos_ofertas' | 'conexao'
  const [activeTab, setActiveTab] = useState<'financeiro' | 'planos_ofertas' | 'conexao'>('financeiro');
  ...
  const handleEnviarSegundaVia = async (fatura: SgpFatura, canal: 'whatsapp' | 'email' = 'whatsapp') => {
    ...
    try {
      ...
      if (res.ok && data.sucesso) {
        ...
        alert(data.erro || 'Não foi possível enviar a 2ª via.');
      }
    } catch (err) {
      console.error('Erro ao enviar 2ª via:', err);
      alert('Erro de comunicação com o servidor SGP.');
    } finally {
  ...
  const handleConfirmarRegistroOferta = async () => {
    ...
    try {
      ...
      if (res.ok && data.sucesso) {
        ...
        alert(data.erro || 'Falha ao registrar oferta no SGP.');
      }
    } catch (err) {
      console.error('Erro ao registrar oferta:', err);
      alert('Erro de comunicação ao registrar oferta no SGP.');
    } finally {
  ...
  return (
    <div className={`bg-white rounded-2xl border border-slate-200  overflow-hidden flex flex-col ${className}`}>
      ...
      <div className="p-4 sm:p-5 border-b border-slate-200 bg-slate-50/80">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-blue-600/10 border border-blue-200 flex items-center justify-center text-blue-600 font-bold">
              <Zap size={16} />
            </div>
            <div>
        ...
        {errorMsg && (
          <div className="mt-2.5 p-2 bg-amber-50 border border-amber-200 rounded-lg flex items-center gap-2 text-xs text-amber-800">
      ...
      {segundaViaSucesso && (
        <div className="mx-4 sm:mx-5 mt-4 p-3 bg-emerald-50 border border-emerald-200 rounded-xl flex items-start justify-between gap-3 text-xs text-emerald-900 animate-in fade-in">
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <div className="w-11 h-11 rounded-xl bg-blue-100 text-blue-700 font-bold flex items-center justify-center text-sm border border-blue-200">
                  {cliente.nome.split(' ').map(n => n[0]).slice(0, 2).join('')}
                </div>
                <div>
                  <div className="flex items-center gap-2">
                    <h4 className="font-bold text-slate-900 text-sm">{cliente.nome}</h4>
              ...
              {cliente.faturas && cliente.faturas.length > 0 ? (
                <div className="divide-y divide-slate-100 border border-slate-200 rounded-xl overflow-hidden bg-white">
                  {cliente.faturas.map((fatura) => (
                    <div key={fatura.id} className="p-3.5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:bg-slate-50/70 transition-colors">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500 mt-1">
                            Vencimento: <span className="font-semibold text-slate-700">{fatura.vencimento}</span> • Valor: <span className="font-bold text-slate-900">R$ {fatura.valor.toFixed(2)}</span>
                            {fatura.pago_em && (
                              <span className="text-emerald-700 font-medium ml-1">({fatura.pago_em})</span>
                            )}
                          </p>
                          ...
                          {fatura.status !== 'pago' && fatura.pix_copia_cola && (
                            <div className="mt-2 flex flex-wrap items-center gap-2 text-[11px]">
                              <button
                                ...
                                className="flex items-center gap-1 text-slate-600 hover:text-blue-600 bg-slate-100 hover:bg-blue-50 px-2 py-1 rounded border border-slate-200 transition-colors"
                                title="Copiar PIX Copia e Cola"
                              >
                                {copiedPixId === String(fatura.id) ? (
                                  <>
                                    <Check size={12} className="text-emerald-600" />
                                    <span className="text-emerald-700 font-bold">PIX Copiado!</span>
                                  </>
                                ) : (
                                  <>
                                    <Copy size={12} />
                              ...
                              {fatura.link_pdf && (
                                <a 
                                  ...
                                  target="_blank" 
                                  rel="noreferrer"
                                  className="flex items-center gap-1 text-slate-600 hover:text-slate-900 bg-slate-100 px-2 py-1 rounded border border-slate-200"
                                >
                                  <Download size={12} />
                    ...
                    >
                      <div>
                        ...
                        <div className="flex items-baseline gap-2 mb-3">
                          ...
                          <span className="text-[11px] text-emerald-700 font-semibold ml-auto font-mono">
          ...
          {activeTab === 'conexao' && cliente.conexao && (
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs">
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200">
                <span className="text-[10px] uppercase font-bold text-slate-400 block mb-1">Status Radius/PPPoE</span>
              ...
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200">
                <span className="text-[10px] uppercase font-bold text-slate-400 block mb-1">Sinal Óptico (ONU)</span>
                <span className="font-bold text-slate-800 font-mono">
                  {cliente.conexao.sinal_optico}
                </span>
              </div>
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200">
                <span className="text-[10px] uppercase font-bold text-slate-400 block mb-1">Uptime de Conexão</span>
                <span className="font-bold text-slate-800 font-mono">
                  {cliente.conexao.uptime}
                </span>
              </div>
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 col-span-2 sm:col-span-2">
                <span className="text-[10px] uppercase font-bold text-slate-400 block mb-1">OLT / PON / Porta CTO</span>
                <span className="font-bold text-slate-800 font-mono text-[11px]">
                  {cliente.conexao.olt_pon}
                </span>
              </div>
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200">
                <span className="text-[10px] uppercase font-bold text-slate-400 block mb-1">Concentrador BNG</span>
                <span className="font-bold text-slate-800 font-mono">
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in">
          <div className="bg-white rounded-2xl border border-slate-200  max-w-md w-full p-5 sm:p-6 space-y-4">
            ...
            <div className="p-3.5 bg-slate-50 border border-slate-200 rounded-xl space-y-2 text-xs">
              ...
              <div className="flex justify-between items-center">
                <span className="text-slate-500 font-semibold">Plano Atual:</span>
                <span className="text-slate-600 font-mono">{cliente.plano_atual?.nome} (R$ {cliente.plano_atual?.valor?.toFixed(2)})</span>
              </div>
              <div className="flex justify-between items-center border-t border-slate-200 pt-2">
                <span className="text-blue-700 font-bold">Novo Plano Ofertado:</span>
                <span className="font-bold text-blue-700">{selectedPlanoParaOferta.nome}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-500 font-semibold">Novo Valor Mensal:</span>
                <span className="font-bold text-emerald-700 text-sm">R$ {selectedPlanoParaOferta.valor.toFixed(2)}/mês</span>
              </div>
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                className="w-full p-2.5 bg-white border border-slate-200 rounded-xl text-xs text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-600/40 focus:border-blue-600"

...
export function usePushNotifications() {
  ...
  // Registrar subscrição no backend do NAP
  ...
  const requestPermission = async () => {
    if (!isSupported) {
      alert('Seu navegador atual não possui suporte nativo à API de Notificações.');
      return false;
    }
  ...
  const triggerTestPush = async (payload?: PushNotificationPayload) => {
    const title = payload?.title || 'Portal do Cliente - NAP';

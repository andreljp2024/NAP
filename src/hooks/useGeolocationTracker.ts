...
export function useGeolocationTracker() {
  ...
  const syncLocationWithBackend = useCallback(async (lat: number, lng: number, precisao: number, velocidade: number) => {
    try {
      const usuarioId = user?.id === 'mock-local-id-123' 
        ? (user.role === 'tecnico' ? 3 : user.role === 'operador' ? 2 : 1)
        : (user?.id || 1);
      ...
      await fetch('/api/usuarios/localizacao', {
        ...
        body: JSON.stringify({
          ...
          endereco_estimado: user?.role === 'tecnico' 
  ...
  useEffect(() => {
    ...
    const deveAtivar = user?.role === 'tecnico' || user?.role === 'operador' || user?.role === 'admin' || user?.role === 'superadmin';
    ...
    try {
      watchId = navigator.geolocation.watchPosition(
        ...
        (error) => {
          ...
          syncLocationWithBackend(prev => prev.lat as any, prev => prev.lng as any, 10, 0);
        },
        {

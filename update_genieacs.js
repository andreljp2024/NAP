...
const callGenieAcs = async (path, options = {}) => {
  ...
  if (res.status === 204) return null; // No content for tasks/reboot sometimes
  return await res.json();
};

const mapGenieAcsDeviceToAppFormat = (device) => {
  // A robust mapper that takes raw GenieACS device tree and flattens it
  const getParam = (path) => {
    try {
  ...
  // Power metrics can be scattered, typically in Optical interface
  const rssi = parseFloat(getParam('Device.Optical.Interface.1.LowerReceivePower') || getParam('InternetGatewayDevice.WANDevice.1.WANDSLInterfaceConfig.Status') || '-20'); // Approximation or fallback
  
  const lastInform = device._lastInform || new Date().toISOString();
  // Status is online if last inform was recent (e.g. within 1 hour)
  ...
  return {
    ...
    wifiPassword: '***', // Usually hidden in ACS or requires getParam('Device.WiFi.AccessPoint.1.Security.KeyPassphrase')
...
// Inject helper functions just before the GenieACS mock array

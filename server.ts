...
let genieacsDevices_mock: GenieACSDevice[] = [
  ...
  app.get("/api/genieacs/devices", async (req, res) => {
    try {
  ...
  app.post("/api/genieacs/devices/:id/wifi", async (req, res) => {
    const { id } = req.params;
  ...
  app.get("/api/genieacs/devices/:id/diagnostics", async (req, res) => {
    const { id } = req.params;
  ...
  app.get("/api/sync/status", async (req, res) => {
    ...
    const acsDevicesCount = genieacsDevices_mock.length;
    const acsOnlineCount = genieacsDevices_mock.filter(d => d.status === 'online').length;
    const acsAlarmCount = genieacsDevices_mock.filter(d => d.rssi && d.rssi < -26).length;
  ...
  app.post("/api/sync/executar", async (req, res) => {
    ...
    res.json({
      ...
      detalhes: {
        ...
        genieacs_telemetrias_atualizadas: genieacsDevices_mock.length,
        status: "sincronizado"
      }
    });
  });
